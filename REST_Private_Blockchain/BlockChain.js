...
class Blockchain {
    ...
    constructor() {
        //console.log("Constructing the blockchain");
    ...
    generateGenesisBlock(){
        ...
        this.getBlockHeight().then( (height) => {
            if (height === -1) {
                ...
                this.addNewBlock(new Block.Block('Genesis block')).then((genesis) => {
    ...
    ******************************************************************/
    ...
    /*********************************************************************
     * addNewBlock(block) - method to add a new block to the blockchain at the next height
     * 
     **********************************************************************/
    addNewBlock(block) {
        // Add your code here
...
/***************************************************************
 * getBlockByHash(hash) - gets a block by hash, by calling the Level's getBlockByHash(hash)
 * The block is parsed before returned. For blocks storing star data in the body,
 * the body of the block is still passed stringified
 ***************************************************************/
    getBlockByHash(hash) {
        // Add your code here
        let self = this;
        return new Promise( function(resolve, reject) {
            self.db.getBlockByHash(hash).then((blockDB) => {
                if(blockDB) {
                    let block_parsed = JSON.parse(blockDB);
                    resolve(block_parsed);
                } else {
                    resolve(undefined);
                }
            }).catch((err) => {console.log(err); reject(err);});
...
/*****************************************************************************
 * getBlockByWalletAddress(address) - method returns a star block, or a number of star blocks,
 * that were registered with the same wallet address.
 * Each block is parsed before it's returned.
 * However, the body of each block - where the star data is stored - is still stringified
 ****************************************************************************/
    getBlockByWalletAddress(address){
        let self = this;
        let parsed_list = [];
        return new Promise( function(resolve, reject) {
            self.db.getBlockByWalletAddress(address).then((block_array) => {
                if (block_array && block_array.length > 0)
                {
                    let i = 0;
                    for (i = 0; i < block_array.length; i++) {
                        let block_parsed = JSON.parse(block_array[i]);
                        parsed_list.push(block_parsed);
                    }
                    resolve(parsed_list);
                } else {
                    resolve(undefined);
